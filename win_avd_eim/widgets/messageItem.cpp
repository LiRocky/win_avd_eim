...
#include <QDebug>
#include "base.h"
ChatMessageItem::ChatMessageItem(QWidget *parent) : QWidget(parent)
{
	...
	m_name = "123";
	//m_icon = QPixmap(GetRootPath() + "custom/images/warn_tips.png");
...
void ChatMessageItem::setTextSuccess()
{
	m_loading->hide();
	m_loadingMovie->stop();
	m_isSending = true;
...
void ChatMessageItem::setText(QString text, quint32 time, QSize allSize, ChatMessageItem::User_Type userType)
{
	m_msg = text;
	m_userType = userType;
	m_time = time;
...
QSize ChatMessageItem::fontRect(QString str)
{
	m_msg = str;
	int minHei = 30;
	int iconWH = 40;
...
void ChatMessageItem::setIcon(QString iconPath)
{

}

QSize ChatMessageItem::getRealString(QString src)
{
...
void ChatMessageItem::paintEvent(QPaintEvent *event)
{
	Q_UNUSED(event);
	...
	if (m_userType == User_Type::User_Other) { // 用户
		...
		painter.drawText(this->rect(), 
			QDateTime::fromTime_t(m_time).toString("hh:mm"),
			option);
	}
}

void ChatMessageItem::drawIcon(QPainter* painter)
{
	if (painter == NULL)
	{
		return;
	...
	{
		iconPos = m_iconRightRect;	
	}
	else
	{
		iconPos = m_iconLeftRect;		
	}
	...
	if (m_icon.isNull())
	{
		...
		painter->drawRoundedRect(iconPos, 20, 20);
		...
		painter->drawText(iconPos, Qt::AlignCenter, m_name.left(2));
		painter->restore();
	}
	else
	{
		painter->drawPixmap(iconPos, m_icon);
	}
